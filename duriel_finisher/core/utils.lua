...
function utils.get_altar()
    ...
    for _, actor in pairs(actors) do
        ...
        if distance < 10 then
            if name == get_current_world():get_current_zone_name() or 
                name == (get_current_world():get_current_zone_name():gsub("WT3", "WT4"):gsub("Penitent", "Penitant")) then
                return actor
            end
        end
    end
...
function utils.get_dungeon_entrance()
    ...
    for _, actor in pairs(actors) do
        ...
        local distance = utils.distance_to(actor)
        if distance < 50 then
            if (name == enums.triggers.dungeon_entrance_1 or name == enums.triggers.dungeon_entrance_2) and (get_current_world():get_name() == "Sanctuary_Eastern_Continent" or get_current_world():get_name() == "Frac_Underworld") then
                return actor
            end
        end
    end
